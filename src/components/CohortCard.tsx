...
import CheckIcon from '@mui/icons-material/Check'; // UPDATED: Import Check icon for button
...
export default function CohortCard({ cohort }: CohortCardProps) {
  return (
    ...
    >
      <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ '& .MuiAccordionSummary-content': { my: 'var(--space-md)' } }}>
      ...
      >
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={4}>
            <Typography variant="h6" sx={{ mb: 2 }}>Cohort</Typography>
            <Stack spacing={2}>
              ...
              <Stack direction="row" alignItems="center" spacing={2} sx={{ pt: 1 }}>
                <Typography variant="body2" sx={{ fontWeight: 'var(--font-weight-medium)' }}>{cohort.nextSessionDate}</Typography>
                <Button
                  variant="contained"
                  color="primary"
                  // UPDATED: Added icon with correct size
            ...
            <Stack spacing={2}>
              {cohort.participants.map(p => (
                <Stack key={p.id} direction="row" alignItems="center" spacing={1.5}>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 'var(--font-weight-medium)' }}>{p.name}</Typography>
                </Stack>
              ))}
            </Stack>
            <Button variant="outlined" sx={{ mt: 3 }}>View Participants</Button>
          </Grid>
          ...
          <Grid item xs={12} md={5}>
            ...
            <Stack spacing={2}>
              <Box>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', textAlign: 'right', mt: 0.5 }}>100%</Typography>
              </Box>
              {cohort.sessions.map(s => (
                <Stack key={s.id} direction="row" alignItems="center" spacing={1.5}>
                  <CheckCircleIcon color="primary" />
                  <Typography variant="body2">{s.name}: {s.datetime} {s.duration}</Typography>
                </Stack>
              ))}
            </Stack>
            <Button variant="outlined" sx={{ mt: 3 }}>View Session Notes</Button>
          </Grid>
        </Grid>
      </AccordionDetails>
    </Accordion>
